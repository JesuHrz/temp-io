...
const { auth, devices, notifications, employees, metrics } = require('./api')
...
let services, Agent, Metric, Notification, Employee
    ...
    case '@agent/message':
      ...
      if (clientId.includes('agent:')) {
        try { 
          ...
          const room = `organization:${organizationId}`
          const employee = await Employee.findByCardId(data.cardId)

          if (!employee) {
            io.to(room).emit('card/read', data.cardId)
            return
          }

            io.to(room).emit('card/read', null)
            if (data.temp >= 37) {
